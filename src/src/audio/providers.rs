...
struct SpotifyToken {
    ...
    expires_at: Instant,
    cookie_hash: u64,
    cached_at: Instant,  // Track when the cache entry was created
}
...
pub(crate) async fn get_spotify_session_info(
    ...
    let now = Instant::now();
    const MAX_CACHE_AGE: Duration = Duration::from_secs(86400); // 24 hours - cookies may become stale
    
    {
        ...
        if let Some(ref token) = *cache
            ...
            && now.duration_since(token.cached_at) < MAX_CACHE_AGE
        {
...
pub(crate) async fn get_spotify_client_token_info(
    ...
    let client_version = if let Some(start_pos) =
        html.find("<script id=\"appServerConfig\" type=\"text/plain\">")
    {
    ...
    let ct_data: serde_json::Value = ct_response.json().await.map_err(|e| {
        SerenyaError::Audio(format!("Failed to parse Spotify client token JSON: {e}"))
    })?;
    ...
    let client_token = ct_data
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            SerenyaError::Audio("Spotify client token response missing token".to_owned())
        })?
        .to_owned();
...
async fn fetch_spotify_web_token(
    ...
    match request_spotify_web_token_reasons(http_client, timeout, sp_dc, local_time).await {
        ...
        Err(err) => {
            // Check if the error is due to cookie expiration (401, 403, or invalid_scope)
            let err_str = format!("{:?}", err);
            if err_str.contains("401") || err_str.contains("403") || err_str.contains("invalid") {
                tracing::error!(
                    "Spotify cookie (sp_dc) appears to be expired or invalid. Error: {:?}. Please update your Spotify sp_dc cookie in the config.",
                    err
                );
                // Clear the token cache to force re-authentication on next attempt
                let cache_lock = SPOTIFY_TOKEN_CACHE.get_or_init(|| Mutex::new(None));
                if let Ok(mut cache) = cache_lock.lock().await {
                    *cache = None;
                }
                return Err(SerenyaError::Audio(
                    "Spotify authentication failed - the sp_dc cookie may be expired. Please update your Spotify configuration.".to_owned()
                ));
            }
...
async fn request_spotify_web_token(
    ...
    if !status.is_success() {
        // Extract and log the response body for debugging authentication failures
        ...
        tracing::error!(
            ...
            "Spotify web token request failed"
        );
        return Err(SerenyaError::Audio(format!(
            "Spotify web token request failed with status {}: {}",
            status, error_body
        )));
    }
    ...
    async fn search(
        ...
        for item in items {
            ...
            let url = item
                ...
                .unwrap_or_default();
            let thumbnail = item.album.and_then(|a| a.images).and_then(|mut images| {
                if !images.is_empty() {
                    images.remove(0).url
                } else {
                    None
...
impl YouTubeProvider {
    ...
    async fn search_scrape(
        ...
        let contents = val
            ...
            .ok_or_else(|| {
                SerenyaError::Audio("Failed to extract itemSectionRenderer contents".to_owned())
            })?;
        ...
        for item in contents {
            if let Some(video) = item.video_renderer {
                ...
                let title = video.title.and_then(|t| t.runs).and_then(|mut r| {
                    if !r.is_empty() {
                        r.remove(0).text
                    } else {
                        None
                ...
                let channel = video.owner_text.and_then(|t| t.runs).and_then(|mut r| {
                    if !r.is_empty() {
                        r.remove(0).text
                    } else {
                        None
                ...
                let thumbnail = video
                    ...
                    .and_then(|mut t| if !t.is_empty() { t.remove(0).url } else { None });
...
static SOUNDCLOUD_STATE: LazyLock<RwLock<Option<ClientIdState>>> =
...
pub async fn send_soundcloud_request(
    ...
    {
        tracing::warn!(
            "SoundCloud request unauthorized (401/403). Clearing client_id and retrying once..."
        );
        {
            let mut write_guard = SOUNDCLOUD_STATE.write().await;
            *write_guard = None;
        ...
        if retry_res.status() == reqwest::StatusCode::UNAUTHORIZED
            || retry_res.status() == reqwest::StatusCode::FORBIDDEN
        {
            return Err(SerenyaError::Audio(
                "SoundCloud request unauthorized even after client_id refresh".to_owned(),
            ));
        }
    ...
    async fn search(
        ...
        for entry in search_result.collection {
            let artist = entry
                ...
                .unwrap_or_else(|| "SoundCloud Artist".to_owned());
            candidates.push(TrackCandidate {
                ...
                url: entry
                    .permalink_url
                    .unwrap_or_else(|| format!("https://api.soundcloud.com/tracks/{}", entry.id)),
    ...
    pub async fn load(
        ...
        let resolved: serde_json::Value = res.json().await.map_err(|e| {
            SerenyaError::Audio(format!(
                "Failed to parse SoundCloud resolve response: {}",
                e
            ))
        })?;
        ...
        if kind == "playlist" || kind == "system-playlist" {
            ...
            for chunk in track_ids.chunks(50) {
                ...
                let bulk_tracks: Vec<SoundCloudTrackMetadata> =
                    bulk_res.json().await.map_err(|e| {
                        SerenyaError::Audio(format!(
                            "Failed to parse SoundCloud bulk tracks response: {}",
                            e
                        ))
                    })?;
